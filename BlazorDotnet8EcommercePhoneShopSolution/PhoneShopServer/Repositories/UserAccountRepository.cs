...
namespace PhoneShopServer.Repositories
{
    public class UserAccountRepository : IUserAccount
    {
        ...
        public async Task<LoginResponse> GetRefreshToken(PostRefreshTokenDTO model)
        {
            ...
            var getToken = await appDbContext.TokenInfo.FirstOrDefaultAsync(x => x.RefreshToken == normalToken);
        ...
        {
            var result = await appDbContext.TokenInfo.FirstOrDefaultAsync(_ => _.AccessToken!.Equals(token));
            if (result is null) return null!;
            var getUserInfo = await appDbContext.UserAccounts.FirstOrDefaultAsync(_ => _.Id == result.UserId);
            if (getUserInfo is null) return null!;

            if (result.ExpiryDate < DateTime.Now) return null!;
            var getUserRole = await appDbContext.UserRoles.FirstOrDefaultAsync(_ => _.UserId == getUserInfo.Id);
            if (getUserRole is null) return null!;
            var roleName = await appDbContext.SystemRoles.FirstOrDefaultAsync(_ => _.Id == getUserRole.RoleId);
        ...
        public async Task<LoginResponse> Login(LoginDTO model)
        {
            ...
            var findUser = await appDbContext.UserAccounts.FirstOrDefaultAsync(_ => _.Email!.Equals(model!.Email!));
            if (findUser is null)
        ...
        private async Task SaveToTokenInfo(int userId, string accessToken, string refreshToken)
        {
            ...
            if(getUser is null)
            {
                appDbContext.TokenInfo.Add(new TokenInfo() { UserId = userId, AccessToken = accessToken, RefreshToken = refreshToken });
                await Commit();
            }
            else
            {
        ...
        private async Task<bool> VerifyToken(string refreshToken = null!, string accessToken = null!)
        {
            ...
            {
                var getRefreshToken = await appDbContext.TokenInfo.FirstOrDefaultAsync(_ => _.RefreshToken!.Equals(refreshToken));
                return getRefreshToken is null;
            }
            else
            {
                var getAccessToken = await appDbContext.TokenInfo.FirstOrDefaultAsync(_ => _.AccessToken!.Equals(accessToken));
        ...
        public async Task<ServiceResponse> Register(UserDTO model)
        {
            ...
            {
                var checkIfUserIsCreated = await appDbContext.SystemRoles.FirstOrDefaultAsync(_ => _.Name!.ToLower().Equals("user"));
                int RoleId = 0;
